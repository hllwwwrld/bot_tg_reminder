...
class SqlRequests(DbConnection):
    ...
    def execute_sql(self, sql: str, select: bool = True, fetchone: bool = False) -> Union[tuple, list[tuple]]:
        try:
    ...
    def get_remind_date(self, user_id: str, user_days_remind_to: int = 3) -> list[tuple[str | int]]:
        ...
        {user_days_remind_to} - (date_part('day', age(d.date::date - interval '{user_days_remind_to} days')))\
        as "days_until_date",
        ...
        and {user_days_remind_to} - (date_part('day', age(d.date::date - interval '{user_days_remind_to} days')))\
        between 0 and {user_days_remind_to}
    ...
    def get_remind_hours(self, user_id: str) -> list[tuple[str | int]]:
        sql_request = f'''
        select d."name", d."date", u.user_id, 
        date_part('hour', d."date"::time - '{str(datetime.datetime.now().time())}'::time) as hours_until,
        date_part('minute', d."date"::time - '{str(datetime.datetime.now().time())}'::time) as minutes_until,
        ...
        where 
        case 
            when date_part('hour', d."date"::time - '{str(datetime.datetime.now().time())}'::time) = 0
            then date_part('minute', d."date"::time - '{str(datetime.datetime.now().time())}'::time) >= 0
            when date_part('hour', d."date"::time - '{str(datetime.datetime.now().time())}'::time) = 3
            then date_part('minute', d."date"::time - '{str(datetime.datetime.now().time())}'::time) = 0
            else date_part('hour', d."date"::time - '{str(datetime.datetime.now().time())}'::time) between 1 and 2
        end
        and
        case
